...
function syncCartBadge() {
  const count = state.cart.reduce((sum, item) => sum + Math.max(0, toNumber(item.qty) || 0), 0);
  if (el.cartCount) el.cartCount.textContent = String(count);
  if (el.cartSummaryText) el.cartSummaryText.textContent = `${getSeriesCountLabel(count)} في السلة`;
}
...
function getSeriesCountLabel(count) {
  ...
  return `${safeCount.toLocaleString('en-US')} سيري`;
}
