...
const CART_OPEN_QUERY_PARAM = 'cart';
...
function hydrateCartDrawerFromUrl() {
  const url = new URL(window.location.href);
  if (url.searchParams.get(CART_OPEN_QUERY_PARAM) !== '1') return;
  const openWhenReady = () => {
    openDrawer('cart');
...
async function addToCart(productId, qty = 1) {
  ...
  if (!product) return;
  const safeQty = Math.max(1, Math.floor(toNumber(qty) || 1));
  const existing = state.cart.find((item) => item.id === productId);
  const unitPrice = getDisplayPrice(product);
  if (existing) existing.qty += safeQty;
  else state.cart.push({ id: product.id, name: product.name, model: product.model, unitPrice, originalPrice: getSeriesBasePrice(product), pricePiece: getPiecePrice(product), discountPercent: toNumber(product.discountPercent || 0), imageUrl: normalizeImageUrls(product.imageUrls)[0] || '', qty: safeQty, seriesQtyText: getSeriesQtyText(product) });
  renderCart();
  showToast(safeQty > 1 ? `تمت إضافة ${safeQty} سيري` : 'تمت إضافة السيري');
}
