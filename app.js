...
const el = {
  ...
  discoveryWrap: id('discoveryWrap'),
  seasonBrowseLead: id('seasonBrowseLead'),
  seasonExplorer: id('seasonExplorer'),
  prefixStage: id('prefixStage'),
  prefixStageTitle: id('prefixStageTitle'),
  prefixStageLead: id('prefixStageLead'),
  prefixExplorer: id('prefixExplorer'),
  showAllProductsBtn: id('showAllProductsBtn'),
  showSeasonProductsBtn: id('showSeasonProductsBtn'),
  resetDiscoveryBtn: id('resetDiscoveryBtn'),
  toolbarWrap: id('toolbarWrap'),
  productsSectionRow: id('productsSectionRow'),
  productsHeadingTitle: id('productsHeadingTitle'),
  ...
  emptyStateTitle: id('emptyStateTitle'),
  emptyStateText: id('emptyStateText'),
...
function bindUI() {
  ...
  el.resetDiscoveryBtn?.addEventListener('click', resetDiscoveryFlow);
  el.showAllProductsBtn?.addEventListener('click', showAllProductsMode);
  el.showSeasonProductsBtn?.addEventListener('click', () => activateSeasonProducts(getActiveSeason()));
...
function renderMenu() {
  ...
  el.menuCategoryList.appendChild(makeFilterChip('الكل', selectedCat === 'all', () => { state.filter.category = 'all'; state.discovery.category = ''; resetRenderedProducts(); applyFilters(); }));
  getCodeCategoryKeys().forEach((key) => {
    el.menuCategoryList.appendChild(makeFilterChip(getCodeCategoryLabel(key), selectedCat === key, () => { state.filter.category = key; state.discovery.category = key; resetRenderedProducts(); applyFilters(); closeDrawers(); }));
  });
  el.menuSeasonList.innerHTML = '';
  el.menuSeasonList.appendChild(makeFilterChip('الكل', selectedSeason === 'all', () => { state.filter.season = 'all'; state.discovery.season = ''; state.discovery.category = ''; resetRenderedProducts(); applyFilters(); }));
  getSeasonOptions().forEach((season) => {
    el.menuSeasonList.appendChild(makeFilterChip(season, selectedSeason === season, () => { state.discovery.season = season; state.discovery.category = ''; state.filter.season = season; state.filter.category = 'all'; resetRenderedProducts(); applyFilters(); closeDrawers(); }));
  });
...
function renderProducts() {
  const baseItems = state.filteredProducts || [];
  const discoveryOnly = shouldShowDiscoveryOnly();
  const items = discoveryOnly ? [] : baseItems;
  const renderedItems = items.slice(0, state.renderedCount);
  const activeSeason = getActiveSeason();
  const activeCategory = getActiveCategory();
  el.productsGrid.innerHTML = '';
  el.toolbarWrap?.classList.toggle('hidden', discoveryOnly);
  el.productsSectionRow?.classList.toggle('hidden', discoveryOnly);
  if (el.productsHeadingTitle) {
    if (activeSeason && activeCategory) el.productsHeadingTitle.textContent = `موديلات ${getCodeCategoryLabel(activeCategory)} - ${activeSeason}`;
    else if (activeSeason) el.productsHeadingTitle.textContent = `موديلات الموسم ${activeSeason}`;
    else el.productsHeadingTitle.textContent = 'الموديلات';
  }
  if (discoveryOnly) {
    el.visibleCount.textContent = 'اختر موسمًا لعرض الموديلات';
    el.emptyState.classList.add('hidden');
    el.loadMoreWrap.classList.add('hidden');
    return;
  }
  el.visibleCount.textContent = items.length > renderedItems.length ? `${renderedItems.length.toLocaleString('en-US')} من ${items.length.toLocaleString('en-US')} موديل` : `${items.length.toLocaleString('en-US')} موديل`;
  el.emptyState.classList.toggle('hidden', items.length > 0);
  if (el.emptyStateTitle && el.emptyStateText) {
    el.emptyStateTitle.textContent = activeSeason ? 'لا توجد موديلات مطابقة' : 'لا توجد نتائج مطابقة';
    el.emptyStateText.textContent = activeSeason ? 'جرّب بادئة أخرى أو غيّر البحث والتصفية.' : 'جرّب تغيير البحث أو التصفية.';
  }
...
function shouldShowDiscoveryOnly() {
  return !getActiveSeason() && !String(state.filter.search || '').trim() && state.filter.category === 'all' && state.filter.season === 'all' && !state.filter.offersOnly;
}

function getActiveSeason() {
  return state.discovery.season || (state.filter.season !== 'all' ? state.filter.season : '');
}

function getActiveCategory() {
  return state.discovery.category || (state.filter.category !== 'all' ? state.filter.category : '');
}

function showAllProductsMode() {
  state.discovery = { season: '', category: '' };
...
function resetDiscoveryFlow() {
  ...
  resetRenderedProducts();
  applyFilters();
  document.getElementById('discoveryWrap')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function activateSeasonProducts(season) {
  if (!season) return;
  state.discovery.season = season;
...
function activateCategoryProducts(code) {
  const activeSeason = getActiveSeason();
  if (!activeSeason) return;
  state.discovery.season = activeSeason;
  state.discovery.category = code === 'all' ? '' : code;
  state.filter.season = activeSeason;
  state.filter.category = code === 'all' ? 'all' : code;
  state.filter.search = '';
  el.searchInput.value = '';
  resetRenderedProducts();
  applyFilters();
...
function renderDiscoveryHub() {
  if (!el.seasonExplorer || !el.prefixExplorer) return;
  const seasons = getSeasonOptions();
  const activeSeason = getActiveSeason();
  const activeCategory = getActiveCategory();
  const allVisible = getVisibleProducts();
  if (activeSeason && activeCategory) {
    el.heroTitle.textContent = `${getCodeCategoryLabel(activeCategory)} • ${activeSeason}`;
    el.heroSubtitle.textContent = 'تصفح الموديلات التابعة للبادئة المختارة واضغط على أي موديل لفتح تفاصيله في نافذة منبثقة أنيقة.';
    el.heroBadge.textContent = `بادئة ${activeCategory}`;
    el.seasonBrowseLead.textContent = `تم اختيار موسم ${activeSeason}. يمكنك تبديل الموسم في أي وقت أو المتابعة لاختيار بادئة أخرى.`;
  } else if (activeSeason) {
    el.heroTitle.textContent = `موسم ${activeSeason}`;
    el.heroSubtitle.textContent = 'اختر البادئة أو التصنيف المناسب لعرض جميع الموديلات التابعة لهذا الموسم بسرعة.';
    el.heroBadge.textContent = activeSeason;
    el.seasonBrowseLead.textContent = `تم اختيار موسم ${activeSeason}. اختر الآن البادئة المناسبة لعرض الموديلات التابعة لهذا الموسم.`;
  } else {
    el.heroTitle.textContent = 'ابدأ من الموسم المناسب';
    el.heroSubtitle.textContent = 'تصفح المواسم أولاً، ثم اختر البادئة أو التصنيف المناسب للوصول إلى الموديلات بطريقة احترافية سريعة.';
    el.heroBadge.textContent = 'المواسم والبادئات';
    el.seasonBrowseLead.textContent = 'ابدأ بالموسم ثم اختر البادئة أو التصنيف للوصول إلى الموديلات بسرعة.';
  }

  el.seasonExplorer.innerHTML = '';
  seasons.forEach((season) => {
    const seasonProducts = allVisible.filter((item) => String(item.season || '').trim() === season);
    const meta = getSeasonMeta(season);
    const card = document.createElement('button');
    card.className = `season-tile ${activeSeason === season ? 'active' : ''}`;
    card.innerHTML = `
      <span class="season-tile__art ${meta.className}"><i class="${meta.icon}"></i></span>
      <span class="season-tile__content">
        <strong>${escapeHTML(season)}</strong>
        <small>${seasonProducts.length.toLocaleString('en-US')} موديل</small>
        <em>${escapeHTML(meta.caption)}</em>
      </span>
    `;
    card.addEventListener('click', () => activateSeasonProducts(season));
    el.seasonExplorer.appendChild(card);
  });

  const discoveryOnly = shouldShowDiscoveryOnly();
  el.showAllProductsBtn?.classList.toggle('hidden', discoveryOnly);
  el.resetDiscoveryBtn?.classList.toggle('hidden', !activeSeason);
  el.showSeasonProductsBtn?.classList.toggle('hidden', !activeSeason);

  if (!activeSeason) {
    el.prefixStage?.classList.add('hidden');
    return;
  }

  const seasonProducts = allVisible.filter((item) => String(item.season || '').trim() === activeSeason);
  const prefixMap = new Map();
  seasonProducts.forEach((product) => {
    const code = String(product.codeCategory || deriveCodeCategory(product.model)).trim();
    if (!code) return;
    if (!prefixMap.has(code)) prefixMap.set(code, []);
    prefixMap.get(code).push(product);
  });
  const sortedPrefixes = [...prefixMap.keys()].sort((a, b) => a.localeCompare(b, 'en', { numeric: true }));

  el.prefixStage?.classList.remove('hidden');
  el.prefixStageTitle.textContent = `بادئات الموسم ${activeSeason}`;
  el.prefixStageLead.textContent = activeCategory
    ? `${getCodeCategoryLabel(activeCategory)} • ${prefixMap.get(activeCategory)?.length?.toLocaleString('en-US') || '0'} موديل متاح`
    : 'اختر البادئة أو التصنيف لعرض الموديلات التابعة لهذا الموسم.';

  el.prefixExplorer.innerHTML = '';
  const allCard = document.createElement('button');
  allCard.className = `prefix-tile prefix-tile--all ${!activeCategory ? 'active' : ''}`;
  allCard.innerHTML = `
    <span class="prefix-tile__icon"><i class="fa-solid fa-layer-group"></i></span>
    <span class="prefix-tile__content">
      <strong>كل موديلات ${escapeHTML(activeSeason)}</strong>
      <small>${seasonProducts.length.toLocaleString('en-US')} موديل</small>
      <em>عرض كل الموديلات التابعة للموسم</em>
    </span>`;
  allCard.addEventListener('click', () => activateCategoryProducts('all'));
  el.prefixExplorer.appendChild(allCard);

  sortedPrefixes.forEach((code) => {
    const products = prefixMap.get(code) || [];
    const label = getCodeCategoryLabel(code);
    const meta = getPrefixMeta(label, code);
    const card = document.createElement('button');
    card.className = `prefix-tile ${activeCategory === code ? 'active' : ''}`;
    card.innerHTML = `
      <span class="prefix-tile__icon ${meta.className}"><i class="${meta.icon}"></i></span>
      <span class="prefix-tile__content">
        <strong><span class="prefix-tile__code">بادئة ${escapeHTML(code)}</span><span class="prefix-tile__label">${escapeHTML(label)}</span></strong>
        <small>${products.length.toLocaleString('en-US')} موديل</small>
        <em>${escapeHTML(meta.caption)}</em>
      </span>`;
    card.addEventListener('click', () => activateCategoryProducts(code));
    el.prefixExplorer.appendChild(card);
  });
}

function getSeasonMeta(season) {
  const value = String(season || '').trim();
  if (/صيف/i.test(value)) return { icon: 'fa-solid fa-sun', className: 'season-art--summer', caption: 'موديلات خفيفة ومبهجة للموسم الصيفي' };
  if (/شت/i.test(value)) return { icon: 'fa-solid fa-snowflake', className: 'season-art--winter', caption: 'موديلات دافئة بتفاصيل شتوية أنيقة' };
  if (/خريف/i.test(value)) return { icon: 'fa-solid fa-leaf', className: 'season-art--autumn', caption: 'ألوان هادئة وتفاصيل موسمية فاخرة' };
  if (/ربيع/i.test(value)) return { icon: 'fa-solid fa-seedling', className: 'season-art--spring', caption: 'تشكيلة ناعمة بروح الربيع' };
  return { icon: 'fa-solid fa-shirt', className: 'season-art--generic', caption: 'اختيار موسمي مرتب حسب الموديلات' };
}

function getPrefixMeta(label, code) {
  const text = `${label || ''} ${code || ''}`;
  if (/حديثي|مواليد|بيبي|رضع/i.test(text)) return { icon: 'fa-solid fa-baby', className: 'prefix-icon--baby', caption: 'موديلات ناعمة لحديثي الولادة والبيبي' };
  if (/سالوبيت|طقم|بدلة/i.test(text)) return { icon: 'fa-solid fa-shirt', className: 'prefix-icon--set', caption: 'أطقم وموديلات كاملة بتنسيق جاهز' };
  if (/بناتي|فستان/i.test(text)) return { icon: 'fa-solid fa-heart', className: 'prefix-icon--girls', caption: 'موديلات بناتي بتفاصيل ناعمة' };
  if (/ولادي|أولادي|ولد/i.test(text)) return { icon: 'fa-solid fa-star', className: 'prefix-icon--boys', caption: 'موديلات ولادي بطابع عصري' };
  if (/بيجام|نوم/i.test(text)) return { icon: 'fa-solid fa-cloud-moon', className: 'prefix-icon--sleep', caption: 'موديلات راحة ونوم بأقمشة مريحة' };
  return { icon: 'fa-solid fa-tag', className: 'prefix-icon--generic', caption: `تصنيف احترافي للبادئة ${code}` };
}
