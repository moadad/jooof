...
const APP_SW_VERSION = 'joodkids-fast-secure-v18';
...
function renderCategoryManager() {
  ...
  keys.forEach((key) => {
    ...
    wrapper.innerHTML = `<div class="category-code">${escapeHTML(key)}</div><div class="field"><label for="${escapeAttr(inputId)}">الاسم الظاهر</label><input id="${escapeAttr(inputId)}" type="text" value="${escapeAttr(categoryDoc?.label || '')}" placeholder="اسم التصنيف" /></div><button class="ghost-btn" type="button">حفظ</button>`;
...
async function saveCodeCategoryLabel(code, label) {
  ...
  try {
    ...
    if (!cleanLabel) {
      await deleteDoc(doc(db, 'categories', `code-${cleanCode}`));
    ...
    await setDoc(doc(db, 'categories', `code-${cleanCode}`), payload, { merge: true });
...
function getSeriesValue(item) {
  ...
  return round2(getPiecePrice(item) * getPieceQuantity(item));
}
...
function getLinePiecesTotal(item) {
  return getSeriesCount(item) * getPieceQuantity(item);
}
...
function buildWhatsAppOrderMessage(order) {
  ...
  (order.items || []).forEach((item, index) => {
    lines.push(
      ...
      `كمية القطعة: ${getPieceQuantity(item)}`,
      ''
    );
  });
...
async function buildInvoiceCanvas(order) {
  ...
  const cols = [
    ...
    { key: 'pieceQty', title: 'كمية القطعة', width: 150, align: 'center' },
...
function drawInvoiceHeader(ctx, { width, margin, contentWidth, y, order, logoImage, totals, headerHeight }) {
  ...
  ctx.fillText('فاتورة مبيعات', width - margin - 232, y + 90);
...
function drawInvoiceSummary(ctx, { width, margin, contentWidth, y, totals, order, summaryHeight }) {
  ...
  ctx.font = '700 24px Cairo, sans-serif';
  ctx.fillText('الأسعار شاملة تفاصيل السيري والإجماليات', totalX + totalCardWidth - 28, y + 164);
}
