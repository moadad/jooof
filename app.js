...
const APP_SW_VERSION = 'joodkids-fast-secure-v17';
...
function renderAdminOrders() {
  ...
  state.orders.forEach((order) => {
    ...
    const itemsSummary = (order.items || []).map((item) => `${resolveInvoiceProductName(item)} (${item.qty || 0} × ${getSeriesQtyText(item)})`).join(' • ');
    card.innerHTML = `
      ...
      <div class="muted" style="margin-top:8px">${escapeHTML(itemsSummary)}</div>
...
async function submitOrder() {
  ...
  try {
    ...
    const whatsapp = buildWhatsAppLink(buildWhatsAppOrderMessage(savedOrder));
    if (whatsapp) safeOpenExternal(whatsapp);
    state.cart = [];
    renderCart();
...
function buildWhatsAppOrderMessage(order) {
  ...
  (order.items || []).forEach((item, index) => {
    lines.push(
      `${index + 1}) موديل: ${item.model || '—'}`,
      `الكمية: ${item.qty || 0}`,
      `السيري: ${getSeriesQtyText(item)}`,
      `القطع: ${resolveInvoiceProductName(item)}`,
      ''
    );
  });
  lines.push('يرجى من حضرتكم إرسال عربون لتأكيد الطلبية والتواصل معنا.');
...
async function buildInvoiceCanvas(order) {
  ...
  const cols = [
    { key: 'model', title: 'رقم الموديل', width: 220, align: 'center' },
    { key: 'qty', title: 'الكمية', width: 170, align: 'center' },
    { key: 'series', title: 'السيري', width: 210, align: 'center' },
    { key: 'name', title: 'القطع', width: contentWidth - 220 - 170 - 210, align: 'right' },
  ];
  const rowHeights = items.map((item) => {
    measure.font = '600 34px Cairo, sans-serif';
    const lines = wrapCanvasText(measure, resolveInvoiceProductName(item), cols[3].width - 40, 3);
    return Math.max(88, 34 + (lines.length * 40));
  });
  const tableHeight = 78 + rowHeights.reduce((sum, value) => sum + value, 0);
  const height = Math.max(1820, 560 + tableHeight + 210);
  ...
  y = drawInvoiceInfoSection(ctx, { width, margin, contentWidth, y, order });
  y += 30;

  drawInvoiceTable(ctx, {
    width,
    margin,
    contentWidth,
    y,
  ...
  drawInvoiceFooter(ctx, {
    ...
    contentWidth,
    y,
    totalSeries: items.reduce((sum, item) => sum + toInt(item.qty || 0), 0),
...
function drawInvoiceHeader(ctx, { width, margin, contentWidth, y, order, logoImage }) {
  drawRoundedRect(ctx, margin, y, contentWidth, 170, 32, '#ffffff');
  ctx.fill();
  ...
  if (logoImage) {
    ctx.drawImage(logoImage, width - margin - 196, y + 26, 124, 124);
  } else {
    ctx.fillStyle = '#6d28d9';
    ctx.beginPath();
    ctx.arc(width - margin - 130, y + 85, 58, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.font = '900 42px Cairo, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(initials(getBrandDisplayName() || 'JK'), width - margin - 130, y + 98);
  }

  ctx.textAlign = 'right';
  ctx.fillStyle = '#5b2f0b';
  ctx.font = '900 62px Cairo, sans-serif';
  ctx.fillText('فاتورة الطلب', width - margin - 220, y + 82);
  ctx.fillStyle = '#7c6240';
  ctx.font = '700 26px Cairo, sans-serif';
  ctx.fillText(getBrandDisplayName() || SITE_NAME_AR, width - margin - 220, y + 128);
  ctx.textAlign = 'left';
  ctx.fillStyle = '#6b7280';
  ctx.font = '700 22px Cairo, sans-serif';
  ctx.fillText(`رقم الطلب: ${getOrderReference(order)}`, margin + 28, y + 78);
  ctx.fillText(`التاريخ: ${formatOrderDate(order)}`, margin + 28, y + 118);
}

function drawInvoiceInfoSection(ctx, { width, margin, contentWidth, y, order }) {
  const gap = 18;
  const boxWidth = (contentWidth - gap) / 2;
  drawInvoiceInfoCard(ctx, width - margin - boxWidth, y, boxWidth, 150, 'بيانات العميل', [
    `العميل: ${order.customerName || '—'}`,
    `الهاتف: ${order.customerPhone || '—'}`,
    `المدينة: ${order.city || '—'}`,
  ]);
  drawInvoiceInfoCard(ctx, margin, y, boxWidth, 150, 'بيانات الطلب', [
    `العنوان: ${order.address || '—'}`,
...
function drawInvoiceInfoCard(ctx, x, y, w, h, title, lines, align = 'right') {
  ...
  lines.forEach((line, index) => {
    ...
    const yy = y + 78 + (index * 28);
    wrapped.forEach((part, partIndex) => ctx.fillText(part, textX, yy + (partIndex * 28)));
  });
}
...
function drawInvoiceTable(ctx, { width, margin, contentWidth, y, items, cols, rowHeights }) {
  ...
  drawRoundedRect(ctx, startX, y, contentWidth, 78, 22, '#c79b3b');
  ctx.fill();
  ctx.fillStyle = '#fff';
  ...
  cells.forEach((cell) => {
    ...
    ctx.fillText(cell.title, textX, y + 48);
    ctx.strokeStyle = 'rgba(255,255,255,.24)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(cell.x, y + 8);
    ctx.lineTo(cell.x, y + 70);
    ctx.stroke();
  });

  let currentY = y + 78;
  items.forEach((item, index) => {
    const rowHeight = rowHeights[index] || 88;
    drawRoundedRect(ctx, startX, currentY, contentWidth, rowHeight, index % 2 === 0 ? 0 : 0, index % 2 === 0 ? 'rgba(255,255,255,.95)' : 'rgba(255,248,236,.92)');
    ctx.fill();
    ...
    cells.forEach((cell, cellIndex) => {
      ...
      ctx.font = cell.key === 'name' ? '600 34px Cairo, sans-serif' : '700 32px Cairo, sans-serif';
      ctx.textAlign = cell.align === 'right' ? 'right' : 'center';
      const textX = cell.align === 'right' ? cell.x + cell.width - 20 : cell.x + (cell.width / 2);
      if (cell.key === 'name') {
        const lines = wrapCanvasText(ctx, resolveInvoiceProductName(item), cell.width - 40, 3);
        const lineStartY = currentY + 42;
        lines.forEach((line, idx) => ctx.fillText(line, textX, lineStartY + (idx * 36)));
      } else if (cell.key === 'model') {
        ctx.fillText(String(item.model || '—'), textX, currentY + (rowHeight / 2) + 12);
      } else if (cell.key === 'qty') {
        ctx.fillText(String(item.qty || 0), textX, currentY + (rowHeight / 2) + 12);
      } else if (cell.key === 'series') {
        ctx.fillText(getSeriesQtyText(item), textX, currentY + (rowHeight / 2) + 12);
      }
    });
    currentY += rowHeight;
  });
...
function drawInvoiceFooter(ctx, { width, margin, contentWidth, y, totalSeries, totalModels }) {
  drawRoundedRect(ctx, margin, y, contentWidth, 132, 24, 'rgba(255,255,255,.96)');
  ctx.fill();
  ctx.strokeStyle = 'rgba(212,175,90,.34)';
  ctx.lineWidth = 1.4;
  ctx.stroke();
  ...
  ctx.font = '800 28px Cairo, sans-serif';
  ctx.textAlign = 'right';
  ctx.fillText('يرجى من حضرتكم إرسال عربون لتأكيد الطلبية والتواصل معنا.', width - margin - 28, y + 54);
  ctx.fillStyle = '#6b7280';
  ctx.font = '700 24px Cairo, sans-serif';
  ctx.fillText(`عدد الموديلات: ${totalModels}   •   إجمالي الكمية: ${totalSeries}`, width - margin - 28, y + 96);
}
